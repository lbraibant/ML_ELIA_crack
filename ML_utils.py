...
def __patch_rectangle(dict_shape):
    ...
    w = dict_shape["width"]
    h = dict_shape["height"]
    vertice = np.array([[x,x,x+w,x+w],[y,y+h,y+h,y]])
...
def resample_image(image,chopsize=224,offset=(100,100)):
    """
    Resample image and mask (if provided)
    :param image:
    :param chopsize:
    :return:
    """
    width, height = image.size
    cols = list(np.arange(int((width-chopsize)/offset[0])+1)*offset[0])
    cols.append(width-chopsize)
    rows = list(np.arange(int((height-chopsize)/offset[1])+1)*offset[1])
    rows.append(height-chopsize)
